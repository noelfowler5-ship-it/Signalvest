...
import { classifySeries } from "./indicators.js";
...
/**
 * Live/latest-bar wrapper around breakoutVolumeSignals(), for use OUTSIDE a
 * backtest: same no-lookahead rule (BUY on a 20-session-high breakout
 * confirmed by >=1.5x average volume, AVOID on a close back below the
 * 20-session SMA), evaluated only for the most recent bar. Returns null if
 * there isn't enough history yet (same as any other index in the series).
 *
 * This is a SECOND, INDEPENDENT check alongside the live SMA20/50+RSI14
 * trend signal in lib/decision.js — not a replacement for it. Callers pass
 * the result into scoreCandidate({ breakout }) where it's surfaced as an
 * extra positive/risk line on the decision card, deliberately NOT scored
 * (see the comment in lib/decision.js for why).
 */
export function latestBreakoutSignal(bars) {
