 ...
 * @param {{signal:string}|null} [p.breakout] - optional result of
 *   lib/backtest.js's latestBreakoutSignal(hist): an independent 20-day-high
 *   + volume breakout check. Deliberately NOT scored (no weight, doesn't
 *   move `score` or the qualifying bar) — it's a second opinion surfaced as
 *   an extra positive/risk line, not another way to game the same score.
 *   Folding it into the weighted score is a real option later, but that's a
 *   strategy-version decision (see lib/strategy.js), not a default.
 */
export function scoreCandidate({ hist, cls, avgVol20, regime, fundamentals, strategy, breakout = null }) {
  ...
  // Independent breakout check (lib/backtest.js's latestBreakoutSignal) —
  // never scored, see the JSDoc above. Silent when there isn't enough
  // history yet (breakout == null), same as any other unavailable input.
  if (breakout && breakout.signal === "BUY") {
    positives.push("Breakout confirmation — also closed above its 20-day high on ≥1.5x average volume (independent check, not scored)");
  } else if (breakout && breakout.signal === "AVOID") {
    risks.push("Breakout check disagrees — price closed back below its 20-day average (independent check, not scored)");
  }
